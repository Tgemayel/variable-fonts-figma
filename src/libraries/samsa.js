    ...
    if (this.numContours >= 0) {
        ...
        iglyph.components.forEach((component, c) => {
            ...
            for (let p = 0; p < decomp.numPoints; p++) {
                simpleGlyph.points.push([decomp.points[p][0] + dx, decomp.points[p][1] + dy, decomp.points[p][2]]);
            }
...
SamsaGlyph.prototype.instantiate = function (userTuple, instance, extra) {
    ...
    this.tvts.forEach((tvt, t) => {
        ...
        if (S != 0) {
            ...
            if (tvt.iup && this.numContours > 0 && touched.length > 0 && !config.instantiation.ignoreIUP) {
...
SamsaGlyph.prototype.parseTvts = function () {
    ...
    if (size > 0) {
        ...
        if (tuplesSharePointNums) {
            ...
            ps++;
            if (tupleNumPoints & 0x80) (tupleNumPoints = 0x100 * (tupleNumPoints & 0x7f) + data.getUint8(ps)), ps++;
            ...
            while (unpacked.length < tupleNumPoints * 2) {
                ...
                switch (bytesPerNum) {
                    case 0:
                        ...
                        break;
                    case 1:
                        for (r = 0; r < runLength; r++) unpacked.push(data.getInt8(ps)), ps++;
                        break;
                    case 2:
                        for (r = 0; r < runLength; r++) unpacked.push(data.getInt16(ps)), (ps += 2);
                        break;
                }
            }
    ...
    switch (this.curveOrder) {
        ...
        case 3:
            ...
            for (endPt of this.endPts) {
                ...
                else if (contourLen >= 2 && this.points[startPt + 1][2] == 1) firstOnPt = 1;
                else if (contourLen >= 3 && this.points[startPt + 2][2] == 1) firstOnPt = 2;
...
SamsaGlyph.prototype.svg = function () {
    let svg = `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="1000" height="1000" style="background-color: transparent;">
...
function SamsaFont(init, config) {
        ...
        switch (tag) {
            ...
            case 'name':
                ...
                for (let n = 0; n < table.count; n++) {
                    ...
                    if (nameRecordEnd > tableOffset + font.tables['name'].length)
                        // safety check
                        continue;
                        ...
                        case 1:
                            if (nameRecord.languageID == 0)
                                if (nameRecord.encodingID == 0) {
                                    ...
                                    while (p < nameRecordEnd) (str += String.fromCharCode(data.getUint8(p))), p++;
                    ...
                    if (nameRecord.hasOwnProperty('string')) font.names[nameRecord.nameID] = str; // sparse array
            ...
            case 'loca':
                ...
                for (let g = 1; g <= font.numGlyphs; g++) {
                    ...
                    if (indexToLocFormat) font.glyphOffsets[g] = data.getUint32(tableOffset + 4 * g);
                    else font.glyphOffsets[g] = 2 * data.getUint16(tableOffset + 2 * g);
            ...
            case 'fvar':
                ...
                for (let i = 0; i < table.instanceCount + 1; i++) {
                    ...
                    font.axes.forEach((axis, a) => {
                        ...
                        if (i == 0) instance.tuple[a] = axis.default;
                        // user-facing value
            ...
            case 'gvar':
                ...
                for (let t = 0; t < table.sharedTupleCount; t++) {
                    let tuple = [];
                    for (let a = 0; a < table.axisCount; a++) tuple.push(data.getF2DOT14(ps)), (ps += 2);
                ...
                for (let g = 0; g < font.numGlyphs; g++) {
                    ...
                    else font.tupleOffsets[g + 1] = 2 * data.getUint16(tableOffset + 20 + 2 * (g + 1));
            ...
            case 'GSUB':
                if (font.tables['GSUB']) {
                    ...
                    if (table.majorVersion == 1 && table.minorVersion <= 1) {
                        ...
                        if (table.minorVersion == 1) (table.featureVariationsOffset = data.getUint32(p)), (p += 4);
                        ...
                        for (let f = 0; f < table.featureCount && f < 100; f++, p += 6) {
                            ...
                            if (feature.tag !== false) {
                                ...
                                if (feature.featureParams) {
                                    ...
                                    if (feature.nameId > 0) feature.name = font.names[feature.nameId];
                                }
                            ...
                            if (data.getUint16(p) == 1 && data.getUint16(p + 2) == 0) {
                                ...
                                for (let fv = 0; fv < table.featureVariationCount; fv++) {
                                    ...
                                    for (let cd = 0; cd < conditionCount; cd++) {
                                        let p_ =
                                            tableOffset + table.featureVariationsOffset + csOffset + data.getUint32(p);
    ...
    this.parseGlyph = (g) => {
        ...
        // printing glyph
            ...
            if (glyph.numContours > 0) {
                ...
                for (c = 0; c < glyph.numContours; c++) glyph.endPts.push(data.getUint16(p)), (p += 2);
    ...
    this.exportInstance = (instance) => {
            ...
            switch (table.tag) {
                case 'glyf':
                    ...
                    function flushGlyfBuffer(glyfBuffer) {
                        ...
                        if (!glyfBuffer) glyfBuffer = Buffer.alloc(font.config.glyf.bufferSize); // create a new buffer
                    ...
                    for (let g = 0; g < font.numGlyphs; g++) {
                        ...
                        if (glyph.numContours > 0) {
                            ...
                            let maxNewGlyphSize =
                                ...
                                (glyph.numContours + glyph.instructionLength) * 2 +
                                glyph.numPoints * (2 + 2 + 1);
                            ...
                            if (CONFIG.glyf.compression) {
                                ...
                                // write flags
                                for (pt = 0; pt < iglyph.numPoints; pt++) glyfBuffer.setUint8(p, flags[pt]), p++; // TODO: RLE-compress this to save a bit of space
                                ...
                                for (pt = 0; pt < iglyph.numPoints; pt++) {
                                    ...
                                    else glyfBuffer.setInt16(p, dx[pt]), (p += 2);
                                }
                                for (pt = 0; pt < iglyph.numPoints; pt++) {
                                    if (dy[pt] == 0) continue;
                                    if (dy[pt] >= -255 && dy[pt] <= 255)
                                        glyfBuffer.setUint8(p, dy[pt] > 0 ? dy[pt] : -dy[pt]), p++;
                                    else glyfBuffer.setInt16(p, dy[pt]), (p += 2);
                                }
                            } else {
                                // CONFIG.glyf.compression != true
                                ...
                                if (font.config.glyf.overlapSimple) glyfBuffer.setUint8(p, points[0][2] | 0x40); // overlap signal for Apple, see https://developer.apple.com/fonts/TrueType-Reference-Manual/RM06/Chap6AATIntro.html ('glyf' table section)
                            ...
                            // padding
                            if ((glyfBufferOffset + p) % 2) glyfBuffer.setUint8(p, 0), p++;
                            ...
                            if (aws[g] < 0) aws[g] = 0; // gvar may have pushed this negative, as in CrimsonPro-Italic-VariableFont_wght.ttf from wght 400..700
                            lsbs[g] = iglyph.xMin;
                        } // simple glyph end
                        ...
                        // COMPOSITE GLYPH
                            ...
                            for (let c = 0; c < iglyph.components.length; c++) {
                                ...
                                if (c < iglyph.components.length - 1) flags |= 0x0020; // MORE_COMPONENTS
                ...
                case 'name':
                    ...
                    if (!font.names[16] && font.names[1]) newNames.push([16, font.names[1]]);
                        ...
                        switch (tag) {
                            case 'head':
                                let longDateTime =
                                    ...
                                    ((1970 - 1904) * 365 + Math.floor((1970 - 1904) / 4) + 1) * 24 * 60 * 60; // seconds since 1904-01-01 00:00:00... new Date().getTime() is UTC; Math.floor((1970-1904)/4)+1 = 17 leap years between 1904 and 1970
        ...
        if (node) write(fdw, locaBuf, 0, 4 * (font.numGlyphs + 1), newTables['loca'].offset);
        ...
        if (node) write(fdw, hmtxBuf, 0, 4 * font.numGlyphs, newTables['hmtx'].offset);
        ...
        newTableDirectory.forEach((table, t) => {
            for (let c = 0; c < 4; c++) fileHeaderBuf.setUint8(12 + t * 16 + c, table.tag.charCodeAt(c)); // e.g. "hmtx" (maybe should store this as the original 32-bit integer to avoid character conversion)
                    ...
                    if (map[m][1] >= n) {
                        if (map[m][1] == n) {
                            ...
                            if (map[m][1] == map[m - 1][1]) n = map[m - 1][0];
                                ...
                                n =
                                    map[m - 1][0] +
                                    (map[m][0] - map[m - 1][0]) * ((n - map[m - 1][1]) / (map[m][1] - map[m - 1][1]));
                        }
                        break;
                    }
                }
    ...
    this.axisNormalize = (axis, t, avarIgnore) => {
        ...
        if (t == axis.default) n = 0;
            ...
            if (t > axis.max) n = 1;
            else n = axis.max == axis.default ? 0 : (t - axis.default) / (axis.max - axis.default);
        } else {
            if (t < axis.min) n = -1;
            else n = axis.min == axis.default ? 0 : (axis.default - t) / (axis.min - axis.default);
        }
            ...
            for (let m = 0; m < map.length; m++) {
                if (map[m][0] >= n) {
                    if (map[m][0] == n) n = map[m][1];
                        ...
                        n =
                            map[m - 1][1] +
                            (map[m][1] - map[m - 1][1]) * ((n - map[m - 1][0]) / (map[m][0] - map[m - 1][0]));
                ...
                for (let sub of featureVariation.substitutions) {
                    ...
                    for (let lu = 0; lu < lookups.length; lu++) {
                        ...
                        if (gIndex !== -1) {
                            if (lookup.substFormat == 1)
                                // deltaGlyphId method
                                newGlyphId = (g + lookup.deltaGlyphId + 0x10000) % 0x10000;
                            // we + and % 65536 because (g+lookup.deltaGlyphId) can be negative
                            else if (lookup.substFormat == 2)
